...
impl PyDataFrame {
    ...
    pub fn list_of_dicts(&self) -> PyObject {
        Python::with_gil(|py| {
            ...
            let convert_timedelta = utils.getattr("_to_python_timedelta").unwrap();
            ...
            for idx in 0..df.height() {
                ...
                for i in 0..series_len {
                    unsafe
                    {
                        match series[i].get_unchecked(idx) {
                            AnyValue::UInt8(v) => dict.set_item(column_names[i], v),
                            AnyValue::UInt16(v) => dict.set_item(column_names[i], v),
                            AnyValue::UInt32(v) => dict.set_item(column_names[i], v),
                            AnyValue::UInt64(v) => dict.set_item(column_names[i], v),
                            AnyValue::Int8(v) => dict.set_item(column_names[i], v),
                            AnyValue::Int16(v) => dict.set_item(column_names[i], v),
                            AnyValue::Int32(v) => dict.set_item(column_names[i], v),
                            AnyValue::Int64(v) => dict.set_item(column_names[i], v),
                            AnyValue::Float32(v) => dict.set_item(column_names[i], v),
                            AnyValue::Float64(v) => dict.set_item(column_names[i], v),
                            AnyValue::Null => dict.set_item(column_names[i], py.None()),
                            AnyValue::Boolean(v) => dict.set_item(column_names[i], v),
                            AnyValue::Utf8(v) => dict.set_item(column_names[i], v),
                            AnyValue::Utf8Owned(v) => dict.set_item(column_names[i], v),
                            AnyValue::Categorical(idx, rev) => {
                                let s = rev.get(idx);
                                dict.set_item(column_names[i], s)
                            },
                            AnyValue::Date(v) => dict.set_item(column_names[i], convert.call1((v, py_date_dtype)).unwrap()),
                            AnyValue::Datetime(v, tu, tz) => {
                                let nanoseconds = v % 1_000_000_000;
                                let microseconds = (nanoseconds / 1000) as u32;
                                let rust_date = NaiveDateTime::from_timestamp(
                                    (v / 1_000_000_000) as i64,
                                    (nanoseconds) as u32,
                                );
                                let python_date = PyDateTime::new(
                                    py, 
                                    rust_date.year(), 
                                    rust_date.month() as u8,
                                    rust_date.day() as u8,
                                    rust_date.hour() as u8,
                                    rust_date.minute() as u8,
                                    rust_date.second() as u8,
                                    microseconds,
                                    utc
                                ).unwrap();
                                dict.set_item(column_names[i], python_date)
                            },
                            AnyValue::Duration(v, tu) => {
                                dict.set_item(column_names[i], match tu {
                                    TimeUnit::Nanoseconds => convert_timedelta.call1((v, "ns")),
                                    TimeUnit::Microseconds => convert_timedelta.call1((v, "us")),
                                    TimeUnit::Milliseconds => convert_timedelta.call1((v, "ms")),   
                                }.unwrap())
                            },
                            AnyValue::Time(v) => dict.set_item(column_names[i], v),
                            _ => todo!()
                        }.unwrap();   
                    }
                }
